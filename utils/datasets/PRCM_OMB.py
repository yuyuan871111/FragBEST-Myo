...
from .general import holo_data_preparation_recipe
...
def setup_configs(config):
    ...
    config.p_filename = f"PR_{config.traj_name}_aligned"
    config.output_aux_filepath = f"{config.output_path}/pocket_aux.txt"
    config.output_aligned_traj_filepath = (
        f"{config.output_path}/PR_OMB_{config.traj_name}_aligned.xtc"
    )
...
def run_preparation(config):
    ...
    holo_data_preparation_recipe(config)
